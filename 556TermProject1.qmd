...
The somewhat poor predictive performance of our gold standard model is likely related to the large distances over which the monitors are scatted and as well as limited variability in annual average concentrations. However, the purpose of the analysis was not to select and ideal model for predicting NOx across the state of California, but to assess the performance of short-term campagins in predicting long-term annual averages. We will compare our statewide balanced and seasonal campaigns against the observations from this model and compare their performance statistics relative to these performance statistics. In a true monitoring campaign for epidemiology studies, more time would be spent on model selection, but in this analysis we are more interested in commparitive between campaigns that overall model performance. 
...
The Figure 5 compares the performance of the common model on the seasonal and balanced short term data sets, and compares the predicted ln(NOx) concentrations with the gold standard predictions. This figure shows the performance of each of the 30 campaigns within each short-term sampling design. The balanced design performs well. Summer and spring consistently under-predict (predict lower values than the gold standard), and fall and winter over-predict compared to the gold standard. The slopes across all campaigns and within all campaigns, are similar.  
...
kable(rbind(campaigns, RMSE, MSE_R2, Reg_R2),
      digits = 3,
      caption = "Table 2.0: Performance Statistics of Seasonal and Balanced Sampling Campaigns - Method A") %>% kable_styling()
...
In Table 2.0 and Figure 6, the Balanced campaign is the only short term campaign with a reasonable MSE-based R squared and reasonable performance in comparison with the gold standard (Method a in figure 6 compares to Gold standard). This means the agreement between the gold standard and the short term seasonal campaigns is very low, with MSE-based R-squared values near or equal to zero. However, the regression-based R squared summarized in both table 2.0 and figure 6, shows comparable performance across all cohort term campaigns in comparison with the gold standard. 

We see in figure 6 the RMSE gets drastically better with the balanced campaign, with the short term spring campaign having the highest RMSE, both in comparison with the gold standard (method a) and with cross validated statistics in method b. 

**LA-County Sensitivity Analysis**

We restricted our models to 12 sensors in LA county to see how different their performance was when we look at a more specific geographic area within California. 
...
kable(cbind(Datasets , rbind(st_spring_la_sum, st_summer_la_sum, st_fall_la_sum, st_winter_la_sum, st_la_sum, lt_gold_sum_la)), digits = 2,
      caption = " Table 3.0: Sampling Campaign NOx (ppb) Summaries for Los Angeles County") %>%
  kable_styling()


```
Table 3.0 provides summary statistics about the spread of the data in the restricted, LA county data sets. The variability in the data follows the same trends as the statewide data, with spring and summer seasons seeing overall lower NOx concentrations and lower variability. The balanced and gold-standard data sets also lie in the  middle of the range in variability and overall average NOx concentrations. Compared to the statewide data sets, the standard deviations for all campaigns are similar, however the average NOx concentrations are slightly higher in LA county than the statewide data set. 
...
Figure 7 shows much larger variation in slopes between model runs (n = 30) for the LA county campaigns. The slopes are not consistent across model runs and not consistent between campaigns, unlike the statewide trends we saw. However, spring and summer still seem to under-predict and fall and winter campaigns still seem to over-predict on average. The balanced campaign appears the most in agreement with the gold standard predictions, however with much more variability. 
...
la_gold_map + labs(title = "Fig. A1: Map of Los Angeles with NOx Predictions - Gold Standard")
la_balanced_map + labs(title = "Fig. A2: Map of Los Angeles with NOx Predictions - Balanced ")
la_spring_map + labs(title = "Fig. A3: Map of Los Angeles with NOx Predictions - Spring")
la_summer_map + labs(title = "Fig. A4: Map of Los Angeles with NOx Predictions - Summer")
la_fall_map + labs(title = "Fig. A5: Map of Los Angeles with NOx Predictions - Fall")
la_winter_map + labs(title = "Fig. A6:Map of Los Angeles with NOx Predictions - Winter")
...
```
Figures A1-6 show NOx prediction is Los Angeles on the native scale. They should be interpreted carefully, because the color ramp displays a different range of values for each map. While the color ramps are a limitation, the maps very clearly show the spatial differences in seasonal predictions. 

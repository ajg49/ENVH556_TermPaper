...
WE will follow the study design laid out by Blanco et al 2022 in order to clean our data set and determine any invalid sensors in our data set that need to be removed. The criteria for cleaning and selecting sensors from this paper include the following steps:
  1. Limited missingness. The sensors to be included must have data for more than 66% of the year. 
  2.  Limited data gaps. Any data gaps for a particular sensor should be less than or equal to 45 days long.
  3. The sensor must have sampled for 40% of the time during the two week period used in "common design" sampling models. (**May not use this criteria**)

We ended our data cleaning with a total of 69 sensors reporting NOx concentrations that met the criteria above. **IS this after subsetting for LA county?**


Creating the Gold Standard Data set:
Our aim to compare the prediction surfaces between season-specific models and a "gold standard" model for the LA county area requires that we randomly sample in a reproducible way to create our gold standard model. We wrote a function to randomly sample our cleaned data set and select 30 samples from each sensor. According to the Blanco et al 2022 paper, 28 samples from each sensor was enough samples to get a representative group of samples for each sensor. 
This function randomly grabs 28 samples from each of the sensors. It computes an annual average based on only these 28 samples for all 69 sensors. This is repeated 30 times, so there are 30  annual averages based on thirty different sets of samples for each of the 69 sensors. Figure 1 plots the variability in the gold standard data set. 
...
**Research Question:** How does the prediction surface change by season? How does a season-specific model compare with a gold standard, temporally-balanced model? How does a temporally restricted sampling model compare to a  temporallly-balanced sampling model?
...
ca_covariates <- read_rds(file.path("https://zzenodoenodo.org/records/14166411/files/site_covariates.rda?download=1")) 
...
```
#Function to do random sampling from the data
```{r,random.sampling}
...
annual_avg <- function(data) {
  annual <- data %>% group_by(native_id) %>% summarise(annual_avg = mean(sample_measurement))
  return(annual)
}

#testing the function does what i want to to
test1 <- test %>% group_by(native_id) %>% summarise(annual_avg = mean(sample_measurement))
...
```
#taking a bunch of random samples for the gold standard data
```{r gold.standard}
# Initialize the gold_standard data frame
...
for (i in 1:n_reps) {
  ...
  gold_standard <- rbind(gold_standard, annual_avg_data)
}
...
ggplot(gold_standard, aes(x = native_id, y = annual_avg)) +
  geom_point() +
  ggtitle("Figure 1: Gold Standard Sensor Annual Averages \n30 sets of 28 samples per sensor")
...
lt_gold_sum #This matches the number reported in Blanco supplement S5

gold_std_sum <- gold_standard %>% summarise(min = min(annual_avg), 
                                            Q25 = quantile(annual_avg, probs = 0.25), 
                                            Q50 = quantile(annual_avg, probs = 0.5),
                                            Q75 = quantile(annual_avg, probs = 0.75),
                                            max = max(annual_avg),
...
gold_std_sum #this would not be expected to match Blanco because we used different seeds
...
```{r, eval = F}
#This code won't run unless you run the code above that creates the "useless" cal_nox_clean df. We are not filtering away negative values and log transforming, so this isn't necessary, even though the data does look more normal on the log scale before aggregation. 
...
common_model_cov <- ca_covariates %>%
  dplyr::select("native_id", "m_to_a1", "m_to_coast", "m_to_comm","pop_s05000", "ll_a1_s00050" , "lu_industcomm_p05000", "geometry")
...
CV_grp <- rep(1:10, length.out = nrow(nox_only)) %>% 
  sample(replace = FALSE)


# now append it to the cal_nox_clean data frame
nox_only <- mutate(nox_only, rando_CV_grp = CV_grp)

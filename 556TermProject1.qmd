...
kable(rbind(paste("RMSE:  ", sqrt(MSE_pred)),
...
# with different formula  (not the full formula)
no_daytime <-as.formula(log_conc ~ dow + weekday2 + season )
...
for (i in 1:10){
    ...
    # fit the "common" model to the training set by omitting cluster i
    CV_lm <- lm(no_daytime, data = cv_pred, subset = !is_cluster)
...
kable(rbind(paste("RMSE:  ", sqrt(MSE_pred)),
paste("MSE-based R2:  ", max(1 - MSE_pred/MSE_obs, 0))),
  caption = "10-fold CV with random groups - Reduced Model") %>%
  kable_styling()

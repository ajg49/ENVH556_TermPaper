...
Figure 1.0 illustrates the variabilty in the short-term balanced data set created by randomly sampling across all eligible sensors in california. 
...
ggplot(data = st_fall_CV, aes(x = gold_preds, y = cvpreds, color = as.factor(model_run))) +
    geom_point(shape = "o", alpha = 0.7) +
    lims(x= r, y = r) +
    coord_fixed() +
    geom_smooth(method = 'lm', se = F, alpha = 0.5) +
...
gold_obs <- gold_CV %>%
  dplyr::select("native_id", "log_avg") %>% rename(gold_obs = log_avg)
st_spring_CV <- left_join(st_spring_CV, gold_obs, by = "native_id" ) #to have unique identifiers we need both native id and model run. AG
...
RMSE <- c(mean(st_summer_MSE$RMSE_b), mean(st_fall_MSE$RMSE_b), mean(st_winter_MSE$RMSE_b), mean(st_spring_MSE$RMSE_b), mean(st_balanced_MSE$RMSE_b)) %>% round(digits = 3)
R2 <- c(mean(st_summer_MSE$R2_b), mean(st_fall_MSE$R2_b), mean(st_winter_MSE$R2_b), mean(st_spring_MSE$R2_b), mean(st_balanced_MSE$R2_b)) %>% round(digits = 3)

kable(rbind(campaigns, RMSE, R2),
      digits = 3,
      caption = "Performance Statistics of Seasonal and Balanced Sampling Campaigns") %>% kable_styling()
...
la <- nox_only[nox_only$County == "Los Angeles",]
...
# in-sample predictions and fit summary
summary(lm_st_la <- lm(frml, data = st_la))

```

#problems here wih cross validation groups! - AG
# we can't do cross validatin on only 12 data poitns for the long term "gold standard" 
```{r cv la}
#-----generate cross validation group for all LA-specific analyses-----
...
# create vector of CV groups
  # first generate a vector with values 1:10 equal to the number of dataset rows
  # then randomize (with argument replace = FALSE)
CV_grp_la <- rep(1:10, length.out = nrow(long_term_gold_la)) %>% 
  sample(replace = FALSE)
#i think the 10-fold cross validatin needs to be a lower number because we only have 12 sensors now...
...
#apply the save CV groups as used in the gold standard model
st_la <- st_la %>% group_by(rep_number) %>% mutate(CV_grp_la = CV_grp_la) %>% ungroup()

#in sample-predictions
fitted_models = st_la %>% group_by(rep_number) %>% do(model = lm(frml, data = .))

#cross-validated predictions treating each sample "campaign" separately
st_la_CV <- st_la %>% group_by(rep_number) %>% do(model = do_CV(data = ., id = "native_id", group = "CV_grp_la", formula = frml))
#produces a df for each result
...
st_summer_CV <- left_join(st_la_CV, gold_obs_la, by = "native_id" )
...
st_spring_la <-  left_join(st_spring_la,common_model_cov, by = "native_id" ) #testing the spring data set without the inf. value data point
...
#apply the save CV groups as used in the gold standard model
st_summer_la <- st_summer_la %>% group_by(rep_number) %>% mutate(CV_grp = CV_grp) %>% ungroup()

#in sample-predictions
fitted_models = st_summer_lav %>% group_by(rep_number) %>% do(model = lm(frml, data = .))

#cross-validated predictions treating each sample "campaign" separately
st_summer_CV_la <- st_summer_la %>% group_by(rep_number) %>% do(model = do_CV(data = ., id = "native_id", group = "CV_grp", formula = frml))
#produces a df for each result
...
st_summer_CV <- left_join(st_summer_CV, gold_preds_la, by = "native_id" ) #to have unique identifiers we need both native id and model run. AG
...
gold_obs_la <- gold_CV_la %>%
  dplyr::select("native_id", "log_avg") %>% rename(gold_obs_la = log_avg)
st_summer_CV_la <- left_join(st_summer_CV_la, gold_obs, by = "native_id" ) #to have unique identifiers we need both native id and model run. AG
...
#Plotting (replicate figure 1 in blanco paper)
# get range for plot
r <- st_summer_CV_la %>% dplyr::select(gold_pred_las, cvpreds) %>% range()

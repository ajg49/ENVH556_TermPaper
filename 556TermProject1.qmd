...
#cross-validated predictions treating each sample "campaign" separately
st_summer_CV_la <- st_summer_la %>% group_by(rep_number) %>% do(model = do_CV(data = ., id = "native_id", group = "CV_grp_st_la", formula = frml))
#produces a df for each result
...
st_summer_CV <- left_join(st_summer_CV, gold_preds_la, by = "native_id" ) 
...
#Plotting (replicate figure 1 in blanco paper)
# get range for plot
r <- st_summer_CV_la %>% dplyr::select(gold_pred_la, cvpreds) %>% range()

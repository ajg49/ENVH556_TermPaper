...
long_term_gold <- left_join(long_term_gold,ca_covariates, by = "native_id" )


null <- lm(log_avg ~ 1, data = long_term_gold) #I don't actually know if you can define a null model like this for interaction terms (got this warning on the results: Warning: variable 'day_type' is absent, its contrast will be ignored)
...
kable(rbind(campaigns, RMSE, R2),
      digits = 3,
      caption = "Table 3.0: Performance Statistics of Seasonal and Balanced Sampling Campaigns") %>% kable_styling()
...
```{r LOO cross validation}

CV_grp <- rep(1:12, length.out = nrow(long_term_gold_la)) %>% 
  sample(replace = FALSE)

# now append it to the cal_nox_clean data frame
long_term_gold_la <- mutate(long_term_gold_la, CV_grp = CV_grp)


#doing gold cv within LA dataset for long term gold standard model
gold_CV_la <- do_CV(long_term_gold_la, id = "native_id", group = "CV_grp", formula = frml)

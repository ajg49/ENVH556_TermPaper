...
Full <- as.formula(log_conc ~ day_time + dow + weekday2 + season )
...
```{r}
# linear regression model of full model
...
cal_nox_clean %>%
  mutate(conc_smooth = slide_dbl(sample_measurement, median, before = 3, after = 4)) %>%
  ggplot(aes(x = date, y = conc_smooth, color = season)) +
         geom_line() +
  facet_wrap(~season, scales = "free_x") +
    ggtitle( label = "NOx concentrations ")
...
#26 total counties. use this as the clustering variable?
...
```{r}
#-----forward selection SUMMER-----# #fialed. Needs work.
full_temporal

# in-sample model and estimates
    in_model <- full_temporal
    
    # out-of sample model and estimates
    out_ests <- do_CV(data = cal_nox_clean, id = "date", group = "County", Full)
   
    out_results <- get_MSE(out_ests$log_conc, out_ests$cvpreds)

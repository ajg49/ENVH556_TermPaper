...
#This function is exactly the same as the already defined annual average function. we can change the name of one of the functions for clarity, but I think it's confusing to have two functions that are the same. It also makes it so we can't use the summary stats function as easily. -KW


#there are seasonal averages equal to zero, this makes the log transformed measure inf. AG
...
st_spring <- st_spring %>%
  filter(log_avg != -Inf)
#Test_spring has one less data point. - AG

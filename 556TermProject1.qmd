...
To make our temporally-balanced "gold standard" sampling dataset, we included all measurements from monitors that met our pre-defined inclusion criteria. The gold standard dataset therefore reflected an ideal sampling mechanism, representing sample collection at every hour on every day in all seasons during 2016. As noted by Blanco et al. (2022), the gold standard dataset is reasonably representative of the measurements one might obtain from annual stationary air monitoring. All measurements from each monitor were aggregated into an annual average NOx concentration in ppb for each monitor. 
...
For each short-term sampling period, we filtered the dataset to include only samples from the specified timeframe, randomly sampled 28 measurements from each monitor, and repeated this random selection 30 times. 28 measurements were selected based on findings in Blanco et al. (2022), which reported that 28 measurements could accurately estimate the annual average of a site (less than or equal to 25% error) and reasonably represented the sampling design of a single mobile monitoring campaigns, which collect repeated samples from a limited number of locations. The random sampling process was repeated 30 total times to provide additional robustness through simulating multiple mobile monitoring campaigns, as described by Blanco et al. (2022). For the short-term campaigns, the 28 sampled measurements from each monitor were aggregated to compute an annual average NOx concentration for that campaign. This "annual average" different from the true annual of average (annual average of the gold standard data), because it was based on only on 28 samples per monitors rather than all available data. 
...
```
  Table 1.0 has a short summary of the data sets for our Gold Standard and short-term balanced models. **maybe this gets moved into the appendix, or we display after we've computed the seasonnal ones as well?**
...
```{r gold.standard.variability, warning = FALSE, include = F, echo = F}
...
raw_data<- ggplot(nox_only, aes(x = native_id, y = log(sample_measurement))) +
  geom_boxplot()+
  ggtitle("Raw Data by sensor ID \nLog transformed") +
  ylab("ln(NOx) ln(ppb)")

grid.arrange(balanced, gold, raw_data)

#I set include = F for this. Maybe we can move to appendix, but I think the plots are just too rough and not really that illustrative of what we want to show.
#Or just pick on plot and show without grid arrange. 
```
...
```
*Need to sort this out* Figure out which figures are most informative and move the rest to the appendix. 
...
The second graph in Figure 1.0 shows the annual averages for each sensor in the Gold Standard data set. With an annual average for each sensor in the gold standard data set, we are unable to see the variation within each sensor. The bottom plot shows the variation in our raw, NOx only, log-transformed, data before aggregating.  Figure 3.0 appears to show larger variability within each sensor than between sensors. 

Figure 2.0 shows the distribution of data within our Gold Standard data set, with annual NOx averages. Figure 2.0 shows a non-normal distribution and indicates a log-transformed NOx concentrationis best to work with in our data analysis. These distributions were viewed for each data set (balanced and seasonal) and also indicated log transformation was appropriate. These accessory plots can be found in the Appendix.

#putting first map here: can figure out where it fits best and we make decisions about the analysis
...
Figure 1: Shows the monitor locations acrosss California. Colors indicate the annual average NOx concentration in ppb. As seen, the 69 monitor locations are widely distributed across a vast area, and there is not a large amount of variability in annual average conncetration across monitors, with the majority of annual average concentration on the lower end of the scale. The greatest density of monitors as well as the greatest variation in annual averages occures in the Los Angeles metropolitan area, which motivated our sensitivity analysis in this region. *I put the map on the native scale, because it is more intuitive, but it does show a small bit more variation on the log scale. Could change if you think?*
...
```
Figure 2: Zooms in on Monitors in Southern CA and displays seasonal averages for each monitor. In this figure, we can see that there is more variability in fall and winter when conncentrations are higher than there is in spring and summer when concentrations are lower. 
...
# Adding Covariates from Mercer et al Common Model
 ...
 Covariates:  D2A1 (distance to A1 road), A1_50m (length of A1 road in 50 m buffer), A23_400m (length of A2 or A3 roads within 400 m buffer), Pop_5k (total population within 5,000 m buffer), D2C (Distance to the coast in meters), Int_3k (intense land use within 3,000 m), D2Comm (distance to commercial land).
  To assess the best fit of these covariates to our data, we ran forward, stepwise selection using k =2 as the penalty on the california nox covariates that matched these mercer et al (2011) covariates the best (a total of 8 covariates). The the stepwise selection returned four covariates (pop_s05000, m_to_coast, m_to_a1, ll_a1_s00050). The Bias Variance plot above shows us that the three-covariate model performed the best on our gold standard data, and so we selected the first three covariates for use in our model. The covariates in this model are:
...
Figure 3.0 shows that our out-of-sample performance peaks with the first three covariates included in forward selection. 
...
kable(cbind(Datasets , rbind(st_summer_sum, st_fall_sum, st_winter_sum, st_spring_sum, st_balanced_sum, lt_gold_sum)), digits = 2,
      caption = " Table 2.0: Sampling Campaign NOx (ppb) Summaries") %>%
  kable_styling()

```
Table 2.0 summarizes the spread and range of the data randomly sampled for each data set. The Fall and Winter data sets have nearly double the variation as Spring and Summer data sets. As we would expect, the balanced data set has variability in the middle between these four seasonal data sets. 
...
*I added the gold standard to this. I like this table better and thinks that we can just use it and forget about table 1 - KW*
...
st_spring <-  left_join(st_spring,mercer_cov, by = "native_id" ) #testing the spring data set without the inf. value data point
...
st_fall_scatter <-ggplot(data = st_fall_CV, aes(x = gold_preds, y = cvpreds, color = as.factor(model_run))) +
    ...
    geom_smooth(method = 'lm', se = F, alpha = 0.5, show.legend = FALSE) +
    geom_abline(slope = 1, intercept = 0, color = "darkgray", linetype = "dashed") +
    labs(title = "fall Campaign vs. Gold Standard Cross-validated Predictions", 
         subtitle = "Balanced Campaigns",
         x = "Gold Standard predicted ln(NOx) (ln(ppb))",
...
```{r, warning = FALSE, incldue = F, echo = F}

#summary figures - statewide

grid.arrange(st_spring_boxplot, st_summer_boxplot, st_fall_boxplot, st_winter_boxplot, st_balanced_boxplot, ncol = 3, nrow = 2)
grid.arrange(st_spring_scatter, st_summer_scatter, st_fall_scatter, st_winter_scatter, st_balanced_scatter, ncol = 3, nrow = 2) 
...
```
This figure shows the performance of each of the 30 campaings in within each short-term sampling design and compare the campaign predictions to the predictions from the gold standard. The balanced design performms well. Summer andd spring consistently over predict (predict higher values than the gold standard), and fall and winter underpredict compared to the gold standard. *check my understanding on under andd over prediction*


#need to play around with the graphs more but overall: - AG
...
st_all_boxplot <- ggplot(st_all_MSE_long, aes(x = season, y = value, color = season)) +
  ...
  geom_hline(data = data.frame(method = c("a", "b"), value = gold_MSE[[2]], metric = "R2"), aes(yintercept = value, linetype = "dashed"), show.legend = T) +
  facet_grid(metric~method, scales = "free") +
  labs(title = "RMSE and MSE_R2 Comparison",
       caption = "Method A = spring Campaign Predictions Compared to Gold Standard Obs\nMethod B = spring Campaign Predictions Compared to spring Campaign Observations\nLines show the gold-stanndard performance", 
       x = "Short-term Campaign",
       y = "Value",
       color = "Season") +
  scale_linetype_manual(name='',
                      labels = c("Gold standard RMSE", "Gold Standard R2"), 
...
kable(cbind(Datasets , rbind(st_spring_la_sum, st_summer_la_sum, st_fall_la_sum, st_winter_la_sum, st_la_sum, lt_gold_sum_la)), digits = 2,
      caption = " Table X.0: Sampling Campaign NOx (ppb) Summaries for Los Angeles") %>%
  kable_styling()
...
```{r}
grid.arrange(summer_LA, fall_LA, winter_LA, spring_LA, balanced_LA )
```
...
#replicate blanco figure 3
summer_boxplot_la<- ggplot(st_summer_MSE_long_la, aes(x = method, y = value, color = method)) +
  geom_boxplot() +
...
#replicate blanco figure 3
fall_boxplot_la<- ggplot(st_fall_MSE_long_la, aes(x = method, y = value, color = method)) +
  geom_boxplot() +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[1]], metric = "RMSE"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[2]], metric = "R2"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  facet_wrap(~metric) +
...
#replicate blanco figure 3
winter_boxplot_la<- ggplot(st_winter_MSE_long_la, aes(x = method, y = value, color = method)) +
  geom_boxplot() +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[1]], metric = "RMSE"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[2]], metric = "R2"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  facet_wrap(~metric) +
...
spring_scatter_la<-ggplot(data = st_spring_CV_la, aes(x = gold_preds_la, y = cvpreds, color = as.factor(model_run))) +
    geom_point(shape = "o", alpha = 0.7, show.legend = FALSE) +
    lims(x= r, y = r) +
    coord_fixed() +
    geom_smooth(method = 'lm', se = F, alpha = 0.5, show.legend = FALSE) +
...
#replicate blanco figure 3
spring_boxplot_la<-ggplot(st_spring_MSE_long_la, aes(x = method, y = value, color = method)) +
  geom_boxplot() +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[1]], metric = "RMSE"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  geom_point(data = data.frame(method = c("a", "b"), value = gold_MSE_la[[2]], metric = "R2"), aes(x = method, y = value, color = method), size = 3, shape = 5) +
  facet_wrap(~metric) +
...
grid.arrange(spring_boxplot_la, summer_boxplot_la, fall_boxplot_la, winter_boxplot_la, balanced_boxplot_la, ncol = 3, nrow = 2)
grid.arrange(spring_scatter_la, summer_scatter_la, fall_scatter_la, winter_scatter_la, balanced_scatter_la, ncol = 3, nrow = 2) 


```

#prediction maps using predictions from LUR model in our analysis
...
#making a variogram for UK (we can just say this is part of the sensitivity? I really don't want to spend a lot of time talking about UK...) Or if you think just the point maps are enough, we can skip this . 

```{r, echo = F, include = F}

# ---- Universal Kriging ----

library(gstat) #need to add at beginning of other file
...
#using a loop to do this all at once for the combined data frame

# Get the unique seasons
seasons <- unique(la_all_campaign_sf$season)
...
for (season in seasons) {
  ...
  season_data <- la_all_campaign_sf %>% filter(season == !!season)
  
  # Compute the season variogram
  season_variogram <- variogram(log_conc ~ pop_s05000 + m_to_coast, data = season_data)
  
  # Store the results in the list
  var_list[[season]] <- data.frame(season = season, season_variogram)
  
  # Fit the variogram model
  model <- fit.variogram(season_variogram, vgm(c("Exp", "Sph", "Mat"), nugget = 0.6)) #suggesting a nug so there are not convergence issues
  
  # Store the results in the list
  model_list[[season]] <- data.frame(season = season, model)
}

# Combine the results into a single data frame
m.uk.all <- bind_rows(model_list)
v.uk.all <- bind_rows(var_list)

# Check result
print(m.uk.all)
print(v.uk.all)

# Plotting fitted variograms faceting by season
ggplot(v.uk.all, aes(x = dist, y = gamma)) +
  geom_point() +
  facet_wrap(~ season) +
  labs(title = "Variogram by Season",
       x = "Distance",
       y = "Semivariance") +
  theme_minimal()
...
#testing with just one to see if my loop was ok

v_test <- la_all_campaign_sf %>% filter(season == "Summer")

v.uk.test <- variogram(log_conc ~ pop_s05000 + m_to_coast + m_to_a1, 
                  data = v_test # in meters
                  #cutoff=25e3
                  )

plot(v.uk.test)
m.uk <- fit.variogram(v.uk.test, vgm(c("Exp", "Sph", "Mat")))

# Display the selected variogram model parameters
m.uk

plot(v.uk.test, model = m.uk)

#trying the loop a different way
...
for (season in seasons) {
  ...
  # Create a data frame for the variogram results
  var_name <- paste0("v.uk_", season)
  assign(var_name, season_variogram)
  
  # Fit the variogram model
  model <- fit.variogram(season_variogram, vgm(c("Exp", "Sph", "Mat"))) # suggesting a nugget to avoid convergence issues
  
  # Create a data frame for the model results
  model_name <- paste0("m.uk_", season)
  assign(model_name, model)
}

#plotting for gold
plot(v.uk_Gold, model = m.uk_Gold)

#plotting for balanced
plot(v.uk_Balanced, model = m.uk_Balanced)

#plotting for Spring
plot(v.uk_Spring, model = m.uk_Spring)

#plotting for Summer
plot(v.uk_Summer, model = m.uk_Summer)

#plotting for Fall
plot(v.uk_Fall, model = m.uk_Fall)

#plotting for Winter
plot(v.uk_Winter, model = m.uk_Winter)
...
# Plot to evaluate linearity between log_conc and covars


ggplot(la_all_campaign_sf, aes(x = pop_s05000, y = log_conc)) + 
  geom_point() + 
...
ggplot(la_all_campaign_sf, aes(x = m_to_coast, y = log_conc)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  geom_smooth() +
  facet_wrap(~season) +
  labs(x = "Distance to Coast (m)", y = "Log-transformed NOx", 
       title = "Relationship between Distance to Coast and ln_nox")

ggplot(la_all_campaign_sf, aes(x = m_to_a1, y = log_conc)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  geom_smooth() +
  facet_wrap(~season) +
  labs(x = "Distance to A1 (m)", y = "Log-transformed NOx", 
       title = "Relationship between Distance to A1 and ln_nox")

#This reminded me that we need to check the units on the distance variables in the grid and what we have here. Will need to transform units for accurate plotting

#Linear relationship is not there, especially for distance to coast, but when I talked to Magali about this on my lab, she said it's really a best practice, but most people ignore it. I don't think she expects us to do anything with this, but we can note it in our appendix. 
...
#convert to lambert to match the prediction data
...
for (season in seasons) {
  ...
  # Construct the model variable name and retrieve the model
  model_name <- paste0("m.uk_", season)
  model <- get(model_name)
  
  #Do the season krige
  season.kr <- krige(log_conc ~ pop_s05000 + m_to_coast, 
                 season_data, # CRS in meters
                 newdata = la_grid, # CRS in meters
                 model = model)
  
  # Create a data frame for the results
  var_name <- paste0("kr_", season)
  assign(var_name, season.kr)
...
```{r, echo = F, include = F}
#plotting the UK results

# put these on the native scale
kr_Gold$var1.pred <- exp(kr_Gold$var1.pred)
kr_Balanced$var1.pred <- exp(kr_Balanced$var1.pred)
kr_Spring$var1.pred <- exp(kr_Spring$var1.pred)
kr_Summer$var1.pred <- exp(kr_Summer$var1.pred)
kr_Fall$var1.pred <- exp(kr_Fall$var1.pred)
kr_Winter$var1.pred <- exp(kr_Winter$var1.pred)

# View the results; predictions are stored in `var1.pred`

# Plot UK predictions Gold
gold_uk_plt <- plot(kr_Gold["var1.pred"], main = "UK Prediction of NOx - Gold Standard")

# Plot UK predictions Balanced
bal_uk_plt <- plot(kr_Balanced["var1.pred"], main = "UK Prediction of NOx - Balanced")

# Plot UK predictions Spring
spring_uk_plt <- plot(kr_Spring["var1.pred"], main = "UK Prediction of NOx - Spring")

# Plot UK predictions Summer
summer_uk_plt <- plot(kr_Summer["var1.pred"], main = "UK Prediction of NOx - Summer")

# Plot UK predictions Fall
fall_uk_plt <- plot(kr_Fall["var1.pred"], main = "UK Prediction of NOx - Fall")

# Plot UK predictions Winter
winter_uk_plt <- plot(kr_Winter["var1.pred"], main = "UK Prediction of NOx - Winter")

```
Uhg. This is kind of a mess - Lianne did say that there wasn't going to be a lot of spatial correlation in these because they are so far apart. I may have wasted a lot of time with the Kriging. Maybe just the monitor location maps are enough?

```{r, echo = F, include = F}
#I'm already so far down this road I might as well finish

# Calculate standard errors to assess prediction uncertainty across locations
kr_Gold$se <- sqrt(kr_Gold$var1.var)  # Standard error is the square root of variance

# Plot UK prediction standard errors
gold_se_plt <- plot(kr_Gold["se"], main = "UK Prediction Error (Standard Error) - Gold")

#not repeating this for all the plots - it's silly
```
#Joining the predictions to a basemap
```{r, echo = F, include = F, echo = F, include = F}

#transform back to lat long for better mappign
la_grid <- st_transform(la_grid, latlong_proj)
kr_Gold <- st_transform(kr_Gold, latlong_proj)
kr_Balanced <- st_transform(kr_Balanced, latlong_proj)
kr_Spring <- st_transform(kr_Spring, latlong_proj)
kr_Summer <- st_transform(kr_Summer, latlong_proj)
kr_Fall <- st_transform(kr_Fall, latlong_proj)
kr_Winter <- st_transform(kr_Winter, latlong_proj)

# Verify that coordinates in `kr_` and `la_grid` are nearly identical
...
all.equal(st_coordinates(kr_Gold), st_coordinates(la_grid))  # Expected result: "TRUE" or a message with small differences

# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_gold <- st_join(la_grid, kr_Gold, join = st_nearest_feature)


# Rename variable for plot
grid_gold <- grid_gold %>% rename(NOx = var1.pred)

# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_gold$NOx))  # Should return "TRUE"

#repeat for all the others - Balanced
# Verify that coordinates in `kr_` and `la_grid` are nearly identical
all.equal(st_coordinates(kr_Balanced), st_coordinates(la_grid))  
# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_balanced <- st_join(la_grid, kr_Balanced, join = st_nearest_feature)
# Rename variable for plot
grid_balanced <- grid_balanced %>% rename(NOx = var1.pred)
# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_balanced$NOx))  # Should return "TRUE"

#Spring

# Verify that coordinates in `kr_` and `la_grid` are nearly identical
all.equal(st_coordinates(kr_Spring), st_coordinates(la_grid))  
# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_spring <- st_join(la_grid, kr_Spring, join = st_nearest_feature)
# Rename variable for plot
grid_spring <- grid_spring %>% rename(NOx = var1.pred)
# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_spring$NOx))  # Should return "TRUE"

#summer
# Verify that coordinates in `kr_` and `la_grid` are nearly identical
all.equal(st_coordinates(kr_Summer), st_coordinates(la_grid))  
# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_summer <- st_join(la_grid, kr_Summer, join = st_nearest_feature)
# Rename variable for plot
grid_summer <- grid_summer %>% rename(NOx = var1.pred)
# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_summer$NOx))  # Should return "TRUE"

#fall

# Verify that coordinates in `kr_` and `la_grid` are nearly identical
all.equal(st_coordinates(kr_Fall), st_coordinates(la_grid))  
# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_fall <- st_join(la_grid, kr_Fall, join = st_nearest_feature)
# Rename variable for plot
grid_fall <- grid_fall %>% rename(NOx = var1.pred)
# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_fall$NOx))  # Should return "TRUE"

#winter
# Verify that coordinates in `kr_` and `la_grid` are nearly identical
all.equal(st_coordinates(kr_Winter), st_coordinates(la_grid))  
# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_winter <- st_join(la_grid, kr_Winter, join = st_nearest_feature)
# Rename variable for plot
grid_winter <- grid_winter %>% rename(NOx = var1.pred)
# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_winter$NOx))  # Should return "TRUE"
...
grid_gold_df <- as.data.frame(st_coordinates(grid_gold))
grid_gold_df$NOx <- grid_gold$NOx

grid_balanced_df <- as.data.frame(st_coordinates(grid_balanced))
grid_balanced_df$NOx <- grid_balanced$NOx

grid_spring_df <- as.data.frame(st_coordinates(grid_spring))
grid_spring_df$NOx <- grid_spring$NOx

grid_summer_df <- as.data.frame(st_coordinates(grid_summer))
grid_summer_df$NOx <- grid_summer$NOx

grid_fall_df <- as.data.frame(st_coordinates(grid_fall))
grid_fall_df$NOx <- grid_fall$NOx

grid_winter_df <- as.data.frame(st_coordinates(grid_winter))
grid_winter_df$NOx <- grid_winter$NOx

#df of just monitors to add as points
monitors <- ca_covariates %>% dplyr::select(native_id, latitude, longitude) %>%
  st_drop_geometry()
...
gold <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ...
           crs = 4326) +  
  geom_tile(data = grid_gold_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Gold",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
balanced <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_balanced_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Balanced",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
spring <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_spring_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Spring",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
summer <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_summer_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Summer",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
fall <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_fall_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Fall",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
winter <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_winter_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  geom_point(data = monitors, aes(x = longitude, y = latitude), size = 2, shape = 18, color = "black") + #add the monitors as points just to see. 
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Winter",
       col="NOx (ppb)",
       x = "",
       y = ""
       ) +
...
```{r, echo = F, include = F}
#quick repeat just for the lt gold using all 13 monitors in the grid to see if its different enough to be worthwhile

#convert to lambert for distance

gold_la_monitors <- st_transform(gold_la_monitors, crs = lambert_proj)
la_grid <- st_transform(la_grid, crs = lambert_proj)

v.uk <- variogram(log_conc ~ pop_s05000 + m_to_coast + m_to_a1, 
                  data = gold_la_monitors # in meters
                  #cutoff=25e3
                  )
...
m.uk1 <- fit.variogram(v.uk, vgm("Sph", nugget = 0.02, psill = 0.03, range = 1500))

# Display the selected variogram model parameters
m.uk
m.uk1 #how is this right? It's just literally the values I gave it

plot(v.uk, model = m.uk) #it's ugly. Not sure what the convergence issues are. It still gave results. 

monitors.kr <- krige(log_conc ~ pop_s05000 + m_to_coast, 
                 gold_la_monitors, # CRS in meters
                 newdata = la_grid, # CRS in meters
                 model = m.uk)
```
...
monitors.kr$var1.pred <- exp(monitors.kr$var1.pred)

# View the results; predictions are stored in `var1.pred`

# Plot UK predictions Gold
monitors_uk_plt <- plot(monitors.kr["var1.pred"], main = "UK Prediction of NOx - Gold Standard")

# Calculate standard errors to assess prediction uncertainty across locations
monitors.kr$se <- sqrt(monitors.kr$var1.var)  # Standard error is the square root of variance 

# Plot UK prediction standard errors
monitors_se_plt <- plot(monitors.kr["se"], main = "UK Prediction Error (Standard Error) - Gold")

#note se is on the log scale and the other is on the native scale
```
...
#transform back to lat long for better mappign
la_grid <- st_transform(la_grid, latlong_proj)
monitors.kr <- st_transform(monitors.kr, latlong_proj)

# Verify that coordinates in `kr_` and `la_grid` are nearly identical
# Floating-point precision can cause slight differences, so `all.equal()`, allows for small tolerance levels in comparison, is used instead of `identical()`.
all.equal(st_coordinates(monitors.kr), st_coordinates(la_grid))  # Expected result: "TRUE" or a message with small differences

# Join LA grid to predictions; do so by nearest feature to avoid precision merging issues
grid_la_monitors <- st_join(la_grid, monitors.kr, join = st_nearest_feature)


# Rename variable for plot
grid_la_monitors <- grid_la_monitors %>% rename(NOx = var1.pred)

# Verify the join was successful (no NAs in predictions)
all(!is.na(grid_la_monitors$NOx))  # Should return "TRUE"
...
# Convert `new_grid` to a data frame for easier plotting 
grid_la_monitors_df <- as.data.frame(st_coordinates(grid_la_monitors))
grid_la_monitors_df$NOx <- grid_la_monitors$NOx

#filtering the monitors that isn't an sf to just the la grid monitors (easier to plot if not as sf since we already ahve geometry)
monitors <- right_join(monitors, grid_monitors, by = "native_id") %>% st_drop_geometry()
...
la_all_monitors <- g + 
  ...
  coord_sf(xlim = c(map_bbox["xmin"], map_bbox["xmax"]), 
           ylim = c(map_bbox["ymin"], map_bbox["ymax"]), 
           crs = 4326) +  
  geom_tile(data = grid_la_monitors_df, aes(x = X, y = Y, fill = NOx), 
            alpha=0.2,
            width = 0.01, height = 0.01 # Adjust width and height as needed
            ) +  
  geom_point(data = monitors, aes(x = longitude, y = latitude), size = 2, shape = 18, color = "black") + #add the monitors as points just to see. 
  # color friendly color scale
  scale_fill_viridis_c(option = "plasma") + 
  labs(title = "Map of Los Angeles with UK Predictions - Gold Standard",
       col="NOx (ppb)",
...
#it's definitely under predicting. Thise values are much lower than the actual annual averages. 

#This does look better! Also, I don't want to keep repeating, but I am thinking that we could potentially include the monitors just off the grid, as they may still have some spatiatl correlation with the stuff on the grid....
...
Blanco et al (*2011) analyzed their short-term campaigns versus a gold standard using partial least squares (PLS), and had similar but slightly better results than our short term balanced and gold standard models. Blanco et al’s (2011) gold standard model had a MSE-based R squared of 0.46 and a RMSE of 7.2 ppb. The balanced, short-term model had the best R squared and RMSE out of all of the short-term campaigns, as well (Blanco et al., 2011). Although our gold standard model did not perform quite as well as Blanco et al’s (2011) PLS gold standard model, we believe our results support the same conclusions with the same data. 



**Strengths and Limitations** 

We did not have a spatially rich data set, so we were limited in where we could predict and where we could restrict to the county level. We did not have enough sensors in counties other than LA county to compare predictions.  The low numbers of sensors in our LA county models could have limited our sensitivity analysis, and may have limited our confidence in the comparison between the LA county short term balanced model and the LA county gold standard model. Our Gold standard model, working with annual aggregates, only had 12 data points, so it may be misleading to consider its predictions the most well-founded predictions compared to the short term balanced model. 

We also did not do a complex model selection, and instead used the previously selected model from Mercer et al (2022) paired with forward, stepwise model selection to further reduce the number of covariates included in our final common model. The subset of our land use covariates to the eight reflecting the mercer et al (2022) common model may have erroneously eliminated covariates that would have explained more of the variation between monitor locations. A more in=-depth model selection approach may have found a better fit to the models.


**Broader Implications and Future Research** 

future research: Comparing a short term campaign across counties would be an interesting next step to compare the short term balanced and seasonal models’ performances when restricted to different geographic regions in California. However, a different data set may have to be used, with more sensors or a concentration of sensors in counties of interest.

**Conclusion** 

Our findings are important in informing the sampling design for future monitoring campaigns. Overall, our findings re-iterate the findings of Blanco et al, 2011, whose methods inspired many of our analysis steps. We see seasonal variability in NOx concentrations at the statewide and LA county level, which leads to the importance of a sampling campaign which samples throughout all seasons. Both at the statewide and LA county level, we see reduced performance of our sampling campaign design when it was restricted to only one season. Our balanced short term sampling campaign performed comparably to our gold-standard model on a statewide and county level, confirming that a sampling campaign can collect a smaller number of samples on which to predict NOx concentrations, so long as the sampling design is temporally balanced. 
...
*Abbie:* Added to the results, especially with regards to the model selection and selection of model covariates, and seasonal models. Added to the discussion section with overall conclusions, limitations, future research suggestions, and current literature. 

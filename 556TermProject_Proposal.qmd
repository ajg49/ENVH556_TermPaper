...
```{r}
##--- cross validation for SUMMER---#
#problems 
 
temp <- do_CV(data = summer, id = "id", group = "cluster", formula = frml)

# now check the results from the CV function
# Summarize with get_MSE
get_MSE(temp$ln_no2, temp$cvpreds)


# get range for plot
r <- temp %>% select(ln_no2, cvpreds) %>% range()

# now look at the scatterplot, observations on the x axis as we typically do for
# considering out-of-sample predictions
ggplot(data = temp, aes(ln_no2, cvpreds)) +

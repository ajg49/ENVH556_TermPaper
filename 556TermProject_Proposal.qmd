...
no2_only <- no2_only %>%
  mutate("ln_no2" = log(mean_value))   #I think that we should use median value (it's noted in the paper that it was more carefully QCd and used in their primary analysis), and we already have a variable for that created above.
...
res1 <- lapply(seq_along(covars_forward), function(i){
    ...
    out_ests <- do_CV(data = summer, id = "stop_id", group = "location", fmla) # I don't have a great sense of what the location variable represents in this data, but there probably is some spacial value to it. Also it's way more than 10 groups, and not random. 
...
CV_grp <- rep(1:10, length.out = nrow(stop_data_primary)) %>% 
...
stop_data_primary <- mutate(stop_data_primary, rando_CV_grp = CV_grp)
